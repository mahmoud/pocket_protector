...
name: release
description: |
	Release pocket_protector to PyPI. Handles version bumping (CalVer YY.MINOR.MICRO),
	tagging, pushing, and post-publish verification. Use when asked to
	"release pocket_protector", "cut a release", "publish to PyPI", or "bump version".
---

# Release pocket_protector

pocket_protector uses CalVer: `YY.MINOR.MICRO` (e.g. `26.0.1`). The version lives in
`pocket_protector/__init__.py` as a `__version__` literal string. During development it
...
3. `pocket_protector/__init__.py` has a `dev` suffix on `__version__`
4. All tests pass: `tox -p auto` (or at minimum `pytest pocket_protector/tests/ -v`)
5. Check what is actually published on PyPI: https://pypi.org/project/pocket-protector/
...
Read `__version__` from `pocket_protector/__init__.py`. Strip the `dev` suffix.
...
Edit `pocket_protector/__init__.py`: remove the `dev` suffix from `__version__`.
...
Add a new section at the top of `CHANGELOG.md` (below the preamble ending at
the CalVer description paragraph) for the release version. Use the existing
RST-style format with underlines and `*` bullet markers:
...
If there are no prior tags, review the full log or diff against the last
CHANGELOG entry.
...
```bash
git commit -am "pocket_protector version 26.0.1"
```

Use the exact format `pocket_protector version X.Y.Z` for the commit message.
...
- `"python 3 support, migrate to face framework"`
- `"fix ruamel.yaml schema validation"`
- `"modernize build system, add OIDC publishing"`
...
```bash
python3 -m venv /tmp/pp-verify && source /tmp/pp-verify/bin/activate
pip install pocket-protector==26.0.1 --index-url https://pypi.org/simple/
cd /tmp  # MUST leave repo root so local pocket_protector/ does not shadow the install
python -c "import pocket_protector; print(pocket_protector.__version__)"
# Should print: 26.0.1
deactivate && rm -rf /tmp/pp-verify
```
...
- **Failed release** (tag exists locally/on GitHub but not on PyPI): PyPI is
	...
	```
	Then check `__version__` in `pocket_protector/__init__.py`. If it was already bumped
